from utils import load_label_file, read_image, show_image, preprocessing_image
...
def write_tfrecords(file_path, image_shape, tfrecord_file):
    ...
    tfrecord_writer = tf.python_io.TFRecordWriter(tfrecord_file + '.record')
...
def batch_test(tfrecord_file, mode='tf'):
    ...
    images_batch, labels_batch = generate_image_label_batch(tf_images, tf_labels, batch_size=1, num_classes=10)
    # open a session 
    with tf.Session() as sess:
        ...
        for i in range(5):
            ...
            print('image shape:{}, image tpye:{}, labels:{}, label type:{}'.format(images.shape, images.dtype, labels, labels.dtype))
